...
export const AuthorizeUser = passport.authenticate("bearer-rule", {
  session: false
});
...
export const AuthenticateUser = (req: Request, resp: Response) => {
  return new Promise<TokenResponse>((resolve, reject) => {
    passport.authenticate(
      "local",
      { session: false },
      async (error, user: ElfUser, info: IVerifyOptions) => {
        if (!!error) {
...
export const GetSignedToken = (
  ...
  rest?: IJwtPayloadUserDetails,
  expiresIn?: string
) => {
  secretOrPrivateKey = secretOrPrivateKey ?? process.env.ELF_AUTH_CIPHER_KEY;
  expiresIn = expiresIn ?? process.env.ELF_AUTH_EXPIRES_IN ?? "7 days";
...
export const AuthorizeUserRoles = (
  req: Request,
...
export function InitializeAuthentication<T extends ElfUser>(
